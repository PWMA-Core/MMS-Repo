...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        // Nexus default: same shape as Input (h-12, rounded-xl, lighter border)
        "border-foreground/15 focus-visible:border-foreground focus-visible:ring-foreground/20 aria-invalid:border-destructive data-[placeholder]:text-foreground/40 [&_svg:not([class*='text-'])]:text-foreground/65 flex w-full items-center justify-between gap-2 rounded-xl border bg-transparent px-4 py-2 text-base whitespace-nowrap transition-colors outline-none focus-visible:ring-1 disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-12 data-[size=sm]:h-10 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    >
